...
uint8_t tensor_arena[kTensorArenaSize];            // Lab8 ML
...
void angle_det(void) {
    ...
    float angle_value;
    float temp1, temp2;
    temp1 = (float)((value[0]*xtest + value[1]*ytest + value[2]*ztest)^2);
    temp2 = (float)((value[0]^2+value[1]^2+value[2]^2)*(xtest^2+ytest^2+ztest^2));
    printf("%.2f %.2f\n", temp1, temp2);
    angle_value = (float)((value[0]*xtest + value[1]*ytest + value[2]*ztest)^2) / (float)((value[0]^2+value[1]^2+value[2]^2)*(xtest^2+ytest^2+ztest^2));
    printf("%.2f\n", angle_value);
    int n = 0;*/
    /*int n = 0;
    while(n < 10) {
      float angle_value;
      float temp1, temp2;
      temp1 = (float)((value[0]*xtest + value[1]*ytest + value[2]*ztest));
      temp2 = (float)(sqrt(value[0]^2+value[1]^2+value[2]^2)*sqrt(xtest^2+ytest^2+ztest^2));
      printf("%.2f %.2f\n", temp1, temp2);
      angle_value = (float)((value[0]*xtest + value[1]*ytest + value[2]*ztest)) / (float)(sqrt(value[0]^2+value[1]^2+value[2]^2)*sqrt(xtest^2+ytest^2+ztest^2));
      printf("%.2f\n", angle_value);
      if (angle_value < cosine[array_index]) {
        printf("tilt success\n");
        ThisThread::sleep_for(500ms);
